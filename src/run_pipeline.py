...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="mfcc",
        choices=["mfcc", "mfcc_frames", "logmelspec", "lyrics_tfidf", "hybrid", "multimodal"],
        help="Feature type",
    )
    p.add_argument(
        "--audio-feature",
    ...
    p.add_argument("--tf-frames", type=int, default=512, help="Target frames for mfcc_frames/logmelspec")
    ...
    p.add_argument("--n-recon", type=int, default=8, help="Number of reconstruction examples")
    p.add_argument(
        "--dist-cols",
        type=str,
        default="language,genre",
...
def main() -> None:
    ...
    def _labels_for(ids_local: list[str]) -> list[str] | None:
        ...
        label_map = {str(row["id"]): row.get(label_col, None) for _, row in df.iterrows()}
    ...
    if feature == "mfcc":
        x, ids = build_feature_matrix_mfcc(repo_root=repo_root, df=df, cache_dir=Path(args.cache))
        ...
        if audio_feat == "mfcc":
            x_a, ids_a = build_feature_matrix_mfcc(repo_root=repo_root, df=df, cache_dir=Path(args.cache))
            ...
            if audio_feat == "mfcc_frames":
                ...
                raise ValueError(f"Unknown audio feature for hybrid: {args.audio_feature}")
        ...
        z_l, ids_l = build_lyrics_embedding_svd(df=df, n_components=int(args.lyrics_embed_dim))
        ...
        if audio_feat == "mfcc":
            ...
            if audio_feat == "mfcc_frames":
                ...
                raise ValueError(f"Unknown audio feature for multimodal: {args.audio_feature}")
        ...
        g, ids_g, _ = build_onehot_from_column(df=df, ids=ids0, col=str(args.genre_col), drop_missing=False)
    ...
    if cond_col:
        c_raw, ids_c, _ = build_onehot_from_column(df=df, ids=ids, col=cond_col, drop_missing=True)
        idx_c = {tid: i for i, tid in enumerate(ids_c)}
        ids2 = [tid for tid in ids if tid in idx_c]
        if not ids2:
            raise ValueError(f"No conditioning values found for --cond-col {cond_col!r}")
    ...
    if args.baseline == "pca":
        if x_std.ndim == 4:
            ...
            raise ValueError(f"PCA baseline expects 2D or 4D features, got rank={x_std.ndim}")
        ...
        if x_std.ndim == 4:
            ...
            raise ValueError(f"Raw baseline expects 2D or 4D features, got rank={x_std.ndim}")
    ...
    for col in dist_cols:
        if col in df.columns:
            ...
            save_cluster_distribution(df=df, ids=ids, clusters=clusters, group_col=col, out_path=out_path)
